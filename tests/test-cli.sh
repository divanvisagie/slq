...
if [[ -f "$PROJECT_DIR/bin/slq" ]]; then
    ...
    echo -e "${RED}Error: slq binary not found. Run 'make' first to build the C version.${NC}"
    exit 1
fi
...
test_help_message() {
    ...
    output=$("$SLQ_BIN" 2>&1) || exit_code=$?
    exit_code=${exit_code:-0}

    assert_exit_code 1 "$exit_code" "Help message exit code" &&
    assert_contains "$output" "Storstockholms Lokaltrafik" "Help message content" &&
...
test_invalid_command() {
    ...
    assert_exit_code 1 "$exit_code" "Invalid command exit code" &&
...
test_missing_arguments() {
    ...
    assert_exit_code 1 "$exit_code" "Missing search argument exit code"
}
